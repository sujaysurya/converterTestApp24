#!/usr/bin/env python3
"""
appian_change_user_password_converter.py
========================================
Converts Appian 'cHANGE User Password' nodes into Java method scaffolds, with full
rule!/cons! drill-down resolution via appian_common.py.

CONFIDENCE NOTE: Well-known plugin operation (original label has irregular casing -- treated as 'Change User Password').
...
NODE_TYPE_LABEL = "cHANGE User Password"
NODE_CATEGORY = "SERVICE_CALL"
EXTRA_CONFIG = {'service_field': 'identityManagementService', 'operation': 'changeUserPassword', 'return_type': 'boolean'}

PARAMETER_SCHEMA = {'user': 'target user', 'newPassword': 'new password (SAIL expr -- handle as a secret, do not log)'}
...
def _demo_node() -> NodeRecord:
    return NodeRecord(
        node_id="demo-change_user_password",
        node_name="Demo cHANGE User Password",
        node_type=NODE_TYPE_LABEL,
        parameters={'user': 'pv!username', 'newPassword': 'pv!tempPassword'},
