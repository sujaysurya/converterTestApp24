#!/usr/bin/env python3
"""
appian_execute_stored_procedure_converter.py
============================================
Converts Appian 'Execute Stored Procedure' nodes into Java method scaffolds, with full
rule!/cons! drill-down resolution via appian_common.py.

CONFIDENCE NOTE: Standard Database smart service -- accurate at a high level; exact IN/OUT parameter shape varies, verify.
...
NODE_TYPE_LABEL = "Execute Stored Procedure"
NODE_CATEGORY = "STORED_PROC"
EXTRA_CONFIG = {}

PARAMETER_SCHEMA = {'procedureName': 'stored procedure name', 'dataSource': 'JDBC connected system', 'parameters': 'SAIL expression mapping IN/OUT parameters'}
...
def _demo_node() -> NodeRecord:
    return NodeRecord(
        node_id="demo-execute_stored_procedure",
        node_name="Demo Execute Stored Procedure",
        node_type=NODE_TYPE_LABEL,
        parameters={'procedureName': '"SP_CALC_INTEREST"', 'parameters': 'a!map(balance: pv!currentBalance, rate: rule!demoBusinessRule(balance: pv!currentBalance))'},
