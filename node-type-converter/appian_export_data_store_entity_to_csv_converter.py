#!/usr/bin/env python3
"""
appian_export_data_store_entity_to_csv_converter.py
===================================================
Converts Appian 'Export DataStore Entity to CSV' nodes into Java method scaffolds, with full
rule!/cons! drill-down resolution via appian_common.py.

CONFIDENCE NOTE: Well-documented Excel/CSV utility plugin -- accurate.
...
NODE_TYPE_LABEL = "Export DataStore Entity to CSV"
NODE_CATEGORY = "SERVICE_CALL"
EXTRA_CONFIG = {'service_field': 'fileProcessingService', 'operation': 'exportDataStoreEntityToCsv', 'return_type': 'Document'}

PARAMETER_SCHEMA = {'dataStoreEntity': 'source entity', 'filterCriteria': 'SAIL filter expression', 'columns': 'columns to export'}
...
def _demo_node() -> NodeRecord:
    return NodeRecord(
        node_id="demo-export_data_store_entity_to_csv",
        node_name="Demo Export DataStore Entity to CSV",
        node_type=NODE_TYPE_LABEL,
        parameters={'dataStoreEntity': '"Account"', 'filterCriteria': 'rule!demoBusinessRule(balance: pv!currentBalance)'},
