#!/usr/bin/env python3
"""
appian_publish_to_kafka_in_bulk_converter.py
============================================
Converts Appian 'Publish to Kafka in Bulk' nodes into Java method scaffolds, with full
rule!/cons! drill-down resolution via appian_common.py.

CONFIDENCE NOTE: Custom/internal Kafka integration plugin -- reasonable generic guess, verify parameter names.
...
NODE_TYPE_LABEL = "Publish to Kafka in Bulk"
NODE_CATEGORY = "KAFKA_PRODUCE"
EXTRA_CONFIG = {'bulk': True}

PARAMETER_SCHEMA = {'topic': 'target Kafka topic', 'key': 'message key (optional)', 'payload': 'list of message payloads'}
...
def _demo_node() -> NodeRecord:
    return NodeRecord(
        node_id="demo-publish_to_kafka_in_bulk",
        node_name="Demo Publish to Kafka in Bulk",
        node_type=NODE_TYPE_LABEL,
        parameters={'topic': '"account-events"', 'payload': 'pv!eventBatch'},
