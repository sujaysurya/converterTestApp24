#!/usr/bin/env python3
"""
appian_user_input_task_converter.py
===================================
Converts Appian 'User Input Task' nodes into Java method scaffolds, with full
rule!/cons! drill-down resolution via appian_common.py.

CONFIDENCE NOTE: Standard Appian node -- accurate structurally; it's a human-in-the-loop boundary, see script comments.
...
NODE_TYPE_LABEL = "User Input Task"
NODE_CATEGORY = "USER_TASK"
EXTRA_CONFIG = {}

PARAMETER_SCHEMA = {'form': 'interface/form design object shown to the user', 'assignedTo': 'SAIL expression for the assignee(s)', 'dueDate': 'SAIL expression for the due date'}
...
def _demo_node() -> NodeRecord:
    return NodeRecord(
        node_id="demo-user_input_task",
        node_name="Demo User Input Task",
        node_type=NODE_TYPE_LABEL,
        parameters={'assignedTo': 'pv!approverUser', 'dueDate': 'rule!demoBusinessRule(balance: pv!currentBalance)'},
