#!/usr/bin/env python3
"""
appian_write_records_and_related_records_converter.py
=====================================================
Converts Appian 'Write Records and Related Records' nodes into Java method scaffolds, with full
rule!/cons! drill-down resolution via appian_common.py.

CONFIDENCE NOTE: Standard Records-based write -- accurate at concept level.
...
NODE_TYPE_LABEL = "Write Records and Related Records"
NODE_CATEGORY = "DATA_STORE_WRITE"
EXTRA_CONFIG = {'multiplicity': 'records'}

PARAMETER_SCHEMA = {'recordType': 'target Record Type', 'recordData': 'SAIL expr for the record to write', 'relatedRecords': 'SAIL expr for related records to write in the same transaction'}
...
def _demo_node() -> NodeRecord:
    return NodeRecord(
        node_id="demo-write_records_and_related_records",
        node_name="Demo Write Records and Related Records",
        node_type=NODE_TYPE_LABEL,
        parameters={'recordType': '"Account"', 'recordData': 'a!map(id: pv!accountId, balance: rule!demoBusinessRule(balance: pv!currentBalance))'},
