#!/usr/bin/env python3
"""
appian_write_to_multiple_data_store_entities_converter.py
=========================================================
Converts Appian 'Write to Multiple Dta Store Entities' nodes into Java method scaffolds, with full
rule!/cons! drill-down resolution via appian_common.py.

CONFIDENCE NOTE: Standard Appian node -- accurate. (Original label has a typo: 'Dta' -> 'Data'.)
...
NODE_TYPE_LABEL = "Write to Multiple Dta Store Entities"
NODE_CATEGORY = "DATA_STORE_WRITE"
EXTRA_CONFIG = {'multiplicity': 'multiple'}

PARAMETER_SCHEMA = {'<entityAlias>': 'one parameter per Data Store Entity being written, each a SAIL expression for its value(s)'}
...
def _demo_node() -> NodeRecord:
    return NodeRecord(
        node_id="demo-write_to_multiple_data_store_entities",
        node_name="Demo Write to Multiple Dta Store Entities",
        node_type=NODE_TYPE_LABEL,
        parameters={'account': 'a!map(id: pv!accountId, balance: rule!demoBusinessRule(balance: pv!currentBalance))', 'auditLog': 'a!map(event: "balance_update", accountId: pv!accountId)'},
